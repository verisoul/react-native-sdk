...
import * as VerisoulModule from '@verisoul_ai/react-native-verisoul';
const Verisoul = (VerisoulModule as any).default || VerisoulModule;
...
export default function App() {
  ...
  const configureSDK = async () => {
    ...
    try {
      console.log(
        'Configuring SDK with ENV:',
        Config.APP_CONFIG,
        'Project ID:',
        Config.VERISOUL_PROJECT_ID
      );
      await Verisoul.configure({
        environment: getEnvironment(Config.APP_CONFIG),
        projectId: Config.VERISOUL_PROJECT_ID,
      });
  ...
  return (
    <VerisoulTouchRootView style={{ flex: 1 }}>
      <SafeAreaView style={styles.container}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          {/* Header */}
          <View style={styles.header}>
            <Text style={styles.headerTitle}>
              Verisoul SDK Test Suite (Expo)
            </Text>
          </View>
